...
from voltron.rollout_utils import GeneratePrediction, Rollouts, volRollout
...
def GenerateStockPredictions_single(ticker, dat, 
                        forecast_horizon=20,
                        train_iters=400, nsample=1000,
                        ntrain=400, mean='ewma', kernel='volt', 
                             save=False, k=300, ntimes=-1, data_num = 0, bm='bm'):
    ...
    for i,last_day in enumerate(end_idxs):
        if i != data_num:
            continue
        else:
            pass
        print("{}th data analyzing!!".format(i+1))
...
#             print("Producing " + ticker + " Forecasts.....")
        if kernel == "volt":
            vol = LearnGPCV(train_x, train_y, train_iters=train_iters,
                                printing=True)
            vmod, vlh = TrainVolModel(train_x, vol, 
                                      train_iters=train_iters, printing=True, kernel = bm)
            voltron, lh = TrainVoltMagpieModel(train_x, train_y[1:], 
                                               vmod, vlh, vol,
                                               printing=True, 
                                               train_iters=train_iters,
            ...
            if mean in ['ewma', 'dewma', 'tewma']:
                ...
                voltron.vol_model.eval()
                if voltron.vol_model.kernel == "bm2" or voltron.vol_model.kernel == "bm3":
                    predvol = volRollout(voltron, train_x, vol, test_x, nsample).exp()
                else:
                    predvol = voltron.vol_model(test_x).sample(torch.Size((nsample, ))).exp()
                save_samples, pred_mean, pred_cov = GeneratePrediction(train_x, train_y, test_x, 
                                             predvol, voltron)
                del predvol
...
#                 torch.save(nans, savepath + model_name + date + ".pt")
    ...
    return dat, save_samples, date, last_day, train_x, train_y, test_x, pred_mean, pred_cov
